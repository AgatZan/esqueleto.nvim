...
---Create template by command
---@param opts Esqueleto.Config
M.create_template = function(opts)
  vim.notify(
    "\nesqueleto :: Entering template creation!",
    vim.log.levels.WARN
  )
  local state = {}
  ...
  if not state.source then
    vim.notify(
      "\nesqueleto :: Exiting template creation!",
      vim.log.levels.WARN
    )
    return nil
  end
  ...
  -- Create paths
  if
    vim.fn.isdirectory(state.directory .. "/" .. state.trigger) == 0
  then
...
M.createexcmd = function(opts)
  ...
  vim.api.nvim_create_user_command(
    "EsqueletoNew",
    function() M.create_template(opts) end,
    {}
  )
end
